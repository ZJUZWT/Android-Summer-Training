...
public class FragmentChatAdapter extends RecyclerView.Adapter<FragmentChatAdapter.MyViewHolder2> {     //一定记得继承的里面是自己的类型
	private List<FragmentChatData> mDataSet = new ArrayList<>();                           //这就是我们的数据列表
	...
	public static class MyViewHolder2 extends RecyclerView.ViewHolder {               //定义自己的ViewHolder
		...
		public void onBind(int position, FragmentChatData data) {    //绑定数据，前面只能算作容器，view什么的都是容器，而内容是string,icon什么的
	...
	public FragmentChatAdapter(List<FragmentChatData> mDataSet_) {                                  //构造函数只需要传递数据，只有数据能够初始化与改变
	...
	public void addData(int position, FragmentChatData data) {                                //添加新的data
	...
	@Override
	public FragmentChatAdapter.MyViewHolder2 onCreateViewHolder(ViewGroup parent, int viewType) {
	...
	public interface IOnItemClickListenerDown {
		void onItemCLickDown(int position, FragmentChatData data);

		void onItemLongCLickDown(int position, FragmentChatData data);
	}
}